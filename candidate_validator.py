...
def nearest_price_after(conn, symbol, target_ts, max_lag_minutes: int | None = None):
    ...
    r=conn.execute('SELECT timestamp, close FROM historical_klines WHERE symbol=? AND timestamp>=? AND timestamp<=? AND close IS NOT NULL ORDER BY timestamp ASC LIMIT 1', (symbol, target_ts.isoformat(), latest_allowed.isoformat())).fetchone()
...
def _validation_from_item(item: dict[str, Any], exchange_info: dict[str, Any] | None) -> SymbolValidationResult:
    ...
    if exchange_info is None:
        return SymbolValidationResult(symbol, False, 'EXCHANGE_INFO_UNAVAILABLE')
...
def validate_candidate(conn, item, freshness=None, now=None, exchange_info=None):
    ...
    symval=_validation_from_item(item, exchange_info)
...
def main(argv=None):
    ...
    with sqlite3.connect(f'file:{sqlite_path(str(DB_PATH))}?mode=ro', uri=True) as conn: results=[validate_candidate(conn,c, freshness) for c in candidates]
