...
class AlphaValidationReportTests(unittest.TestCase):
    ...
    def test_low_sample_regimes_excluded_from_headline_best_worst(self):
        ...
        self.assertEqual(regime["headline_worst"], "UNAVAILABLE")

    def test_two_eligible_regimes_have_distinct_best_and_worst_headlines(self):
        rows = []
        for i in range(1, 26):
            rows.append((i, f"2026-01-{i:02d}", "ETH", "CLOSED", 2.0, "BETTER", 80.0))
        for i in range(26, 51):
            rows.append((i, f"2026-02-{i-25:02d}", "BTC", "CLOSED", -1.0, "WORSE", 80.0))
        report = avr.build_report(self.write_db(rows))
        regime = report["edge_segmentation"]["market_regime"]
        self.assertEqual(regime["headline_best"], "BETTER")
        self.assertEqual(regime["headline_worst"], "WORSE")

    def test_markdown_profit_factor_omits_na_reason_for_valid_numeric_value(self):
        rows = [(1, "2026-01-01", "ETH", "CLOSED", 2.0, "BULL", 90.0), (2, "2026-01-02", "ETH", "CLOSED", -1.0, "BULL", 90.0)]
        report = avr.build_report(self.write_db(rows))
        _, md_path = avr.write_outputs(report)
        text = md_path.read_text()
        self.assertIn("- Profit factor: 2", text)
        self.assertNotIn("- Profit factor: 2 (N/A)", text)

    def test_markdown_profit_factor_shows_explicit_reason_when_unavailable(self):
        rows = [(1, "2026-01-01", "ETH", "CLOSED", 2.0, "BULL", 90.0), (2, "2026-01-02", "ETH", "CLOSED", 1.0, "BULL", 90.0)]
        report = avr.build_report(self.write_db(rows))
        _, md_path = avr.write_outputs(report)
        self.assertIn("- Profit factor: N/A (no_losses)", md_path.read_text())
