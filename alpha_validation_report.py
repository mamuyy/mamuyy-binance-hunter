...
def headline_group(groups: dict[str, dict[str, Any]], reverse: bool = True, require_comparison: bool = False) -> str:
    eligible = [
    ...
    if not eligible or (require_comparison and len(eligible) < 2):
        return "UNAVAILABLE"
    ...
    try:
        ...
        for name, candidates in dims.items():
            ...
            report["edge_segmentation"][name] = {
                ...
                "headline_worst": headline_group(groups, False, require_comparison=True),
            }
...
def format_profit_factor(metrics: dict[str, Any]) -> str:
    profit_factor = metrics.get("profit_factor")
    reason = metrics.get("profit_factor_reason")
    if profit_factor is not None:
        return fmt(profit_factor) if reason is None else f"{fmt(profit_factor)} ({reason})"
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Profit factor: {format_profit_factor(core)}",
