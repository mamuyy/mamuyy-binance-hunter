...
def check_freshness(db_path='mamuyy_hunter.db', candidate_path=Path('reports/binance_candidate_queue.json'), max_age_minutes=None):
    now=datetime.now(timezone.utc); max_age=max_age_minutes or int(os.getenv('DATA_FRESHNESS_MAX_AGE_MINUTES','30'))
    ...
    try:
        with sqlite3.connect(f'file:{db_path}?mode=ro', uri=True) as conn:
            ...
            for sym in symbols:
                ts=_parse(conn.execute('SELECT MAX(timestamp) FROM historical_klines WHERE symbol=?', (sym,)).fetchone()[0])
