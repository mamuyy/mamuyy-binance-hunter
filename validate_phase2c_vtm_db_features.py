#!/usr/bin/env python3
"""Read-only Phase 2C validation for DB-derived VTM features.

Purpose:
- Compare baseline Phase 2C calibration features against baseline + DB VTM
  features from data/ml_calibration_with_vtm_db_features.csv.
...
Output:
- logs/phase2c_vtm_db_features_validation_report.json
"""
...
OUT_PATH = ROOT / "logs/phase2c_vtm_db_features_validation_report.json"
TARGET_BRIER = 0.24
VALIDATION_FRACTION = 0.30

DB_VTM_FEATURE_COLUMNS = [
    "db_volume",
    "db_volume_ratio_20",
    "db_ema20",
    "db_ema200",
...
def load_labeled_rows():
    ...
    missing_vtm = [col for col in DB_VTM_FEATURE_COLUMNS if col not in columns]
    if missing_vtm:
        raise RuntimeError(f"Input CSV missing DB VTM feature columns: {missing_vtm}")
    ...
    for raw in raw_rows:
        ...
        row["_dt"] = dt
        row["_y"] = y
        for col in DB_VTM_FEATURE_COLUMNS:
            row[col] = to_float(row.get(col))
        add_baseline_features(row)
        rows.append(row)

    return sorted(rows, key=lambda r: r["_dt"]), columns, time_column, ignored_labels
...
def make_model(sk):
    ...
    preprocessor = sk["ColumnTransformer"](
        transformers=[("numeric", numeric_pipeline, list(range(0, 9999)))],
        remainder="drop",
    )
...
def main():
    sk = require_sklearn()
    rows, input_columns, time_column, ignored_labels = load_labeled_rows()
    train, valid = time_ordered_split(rows)

    baseline_feature_columns = list(BASELINE_DERIVED_COLUMNS)
    vtm_feature_columns = list(DB_VTM_FEATURE_COLUMNS)
    baseline_plus_vtm_columns = baseline_feature_columns + vtm_feature_columns

    baseline_brier = fit_predict_brier(sk, train, valid, baseline_feature_columns)
    vtm_brier = fit_predict_brier(sk, train, valid, baseline_plus_vtm_columns)
    improvement = baseline_brier - vtm_brier

    report = {
        "build_time_utc": datetime.now(timezone.utc).isoformat(),
        "mode": "READ_ONLY_PAPER_ONLY_PHASE2C_VTM_DB_FEATURE_VALIDATION",
        ...
        "vtm_feature_columns": vtm_feature_columns,
        "baseline_brier": round_or_none(baseline_brier),
        "vtm_brier": round_or_none(vtm_brier),
        "improvement_vs_baseline": round_or_none(improvement),
        "target_brier": TARGET_BRIER,
        "passes_target": bool(vtm_brier <= TARGET_BRIER),
        "label_mapping": {
            "WIN": 1,
            "TP1 HIT": 1,
            "LOSS": 0,
        ...
        "null_rate_per_db_feature": {
            col: round_or_none(null_rate(rows, col)) for col in vtm_feature_columns
        },
        "auc_summary_per_db_feature": {
            col: auc_for_feature(sk, rows, col) for col in vtm_feature_columns
        },
        "safety": {
            "read_only": True,
            "paper_only": True,
